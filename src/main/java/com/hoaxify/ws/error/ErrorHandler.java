...
public class ErrorHandler {
  ...
  ResponseEntity<ApiError> handleException(Exception exception,
    ...
    if(exception instanceof MethodArgumentNotValidException) {
      ...
      var validationErrors = ((MethodArgumentNotValidException)exception).getBindingResult().getFieldErrors().stream().collect(
      ...
      apiError.setStatus(400);
      apiError.setValidationErrors(((NotUniqueEmailException) exception).getValidationError());
...
/*
Bütün rest controller için servis veren bir class a dönüşür. -> RestControllerAdvice
 */